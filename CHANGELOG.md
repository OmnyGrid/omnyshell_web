...
broker on the same port, so a node's shell is one button away.
...
```sh
dart run omnyshell_web:copy_assets   # xterm bundle + stylesheets → your web/
```
...
final shell = WebShellHost(
  term: term, session: session, principal: 'alice', nodeId: id,
  commands: null,  // a plain shell: no :ai, no :ide
);
TerminalFitter(term: term, host: host, accessory: bar, scaleTarget: root).attach();
```

The app itself is unchanged — same screens, same behaviour, same tests, all
passing unmodified. What follows is what had to move to make the terminal
embeddable.

### Added

- **Three public barrels.** `terminal.dart` (the terminal stack), `ui_kit.dart`
  (dom, widgets, toasts, modal) and `client.dart` (`OmnyShellService`,
  `Observable`, `AsyncState`, controllers, storage, router, theme). `app/`,
  `ui/screens/` and `ui/settings_panel.dart` stay app-private — exporting them is
  exactly what would let the shared tier re-acquire `AppContext` coupling. See
  ARCHITECTURE.md → *Public surface*.
- **`TerminalFitter`** — the terminal's sizing engine, lifted out of
  `SessionViewScreen`, where it was ~140 private lines. It is the hardest code in
  the repo (a terminal that grows but never shrinks, renders 0×0, or hides its
  last line behind a soft keyboard is what you get when a piece of it is wrong),
  and no consumer should have to copy it. Both modes are preserved: a *fixed*
  grid whose font rescales (the backend PTY cannot be resized) and *auto-fit*,
  where xterm re-derives cols/rows.
- **`bin/copy_assets.dart`** (`dart run omnyshell_web:copy_assets`) — installs the
  xterm bundle, `kit.css`, `terminal.css` and `boot.js` into a consuming app's
  `web/`. A package ships its `web/` inside the pub archive, but pub never
  *serves* it to a consumer, so without this the terminal loads to a blank box
  and the widgets render unstyled.
- **`XtermTheme`** — the xterm palette, font size and family are now injectable
  (`XtermTerminalView(host, theme: …)`), defaulting to OmnyShell's. xterm paints
  to a canvas and so cannot inherit CSS custom properties; an embedding app has
  to be able to say what its colours are.
- **`example/`** — a whole terminal app in one file, using only the barrels. It
  doubles as a check that the public surface is sufficient without deep imports.
- **LICENSE** (Apache-2.0, matching the rest of OmnyGrid) and a `repository:`
  link, both required to publish.

### Changed

- **Storage prefixes are parameterized**, defaulting to `omnyshell.` so nothing
  changes for this app: `SettingsStore(kv, prefix: …)`, `NodeCache(kv, prefix: …)`,
  `CommandHistory.load(prefix: …)`. `localStorage` is per-origin and one Hub can
  serve two apps, which would otherwise share — and clobber — each other's theme,
  Hub, token and terminal preferences.
- **`styles.css` is split** into `kit.css` (tokens, base, layout, widgets, modal,
  toasts) and `terminal.css` (terminal chrome, key bar, fullscreen, keyboard
  insets), so an app can take the terminal without the whole look, or vice versa.
  Every selector is preserved; the app links both. The service-worker cache
  version is bumped accordingly.
- **`deviceMetrics()` moved** from `ui/settings_panel.dart` to
  `terminal/device_metrics.dart`. It is ten lines of pure `screen` reading, and
  importing the terminal stack should not drag an `AppContext`-coupled settings
  panel along with it. Still re-exported from its old home, so nothing breaks.
